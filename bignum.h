...
public:
	...
	explicit CBigNum(uint512 n) { bn = BN_new(); setuint512(n); }
	explicit CBigNum(uint576 n) { bn = BN_new(); setuint576(n); }
	explicit CBigNum(uint1024 n) { bn = BN_new(); setuint1024(n); }
    ...
    // Provide access to underlying BIGNUM for OpenSSL functions
    ...
    {
        return (unsigned int)BN_get_word(bn);
    }
