...
int main(int argc, char *argv[])
{
	...
	// Port 0 Auto-detect: Try standard ports based on mining mode
	if (PORT == "0")
	{
		if (bSoloMining)
		{
			PORT = "9325"; // Standard solo mining port (mainnet)
			printf("Port auto-detect: Using solo mining port 9325\n");
		}
		else
		{
			PORT = "9549"; // Standard pool mining port
			printf("Port auto-detect: Using pool mining port 9549\n");
		}
	}
	...
	if (bSoloMining && (PORT.compare("9325") != 0 && PORT.compare("8325") != 0 && PORT.compare("0") != 0))
	{
		printf("\n\n!!!!!!! Warning! The miner is in SOLO mode but the specified PORT wasn't set to the standard port number 9325 (8325 on testnet)\n\n");
		printf("Pointing a solo miner to a pool server will result in IP ban from the pool.\n");
		Sleep(5000);
	}
